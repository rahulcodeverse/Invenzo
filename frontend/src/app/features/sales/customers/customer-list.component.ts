      ...
      <ng-container *nzModalContent>
        ...
        <ng-container *ngIf="!statementLoading && statement">
          <div nz-row [nzGutter]="16" class="statement-summary">
            <div nz-col [nzSpan]="6">
              <nz-statistic nzTitle="Total Sales" [nzValue]="statement.summary?.totalSales || 0" nzPrefix="INR "></nz-statistic>
            </div>
            <div nz-col [nzSpan]="6">
...
export class CustomerListComponent implements OnInit {
  ...
  openStatement(item: Customer): void {
    this.statementVisible = true;
    this.statementLoading = true;
    this.statementTitle = `${item.name} Statement`;
    this.service.getCustomerStatement(item.id).subscribe({
      next: (res: any) => {
      ...
      error: () => {
        this.statementLoading = false;
        this.message.error('Unable to load customer statement');
      }
    });
  }
